...
def main():
    ...
    wandb.init(project='renyi', config=def_config)
    config = wandb.config
    ...
    def generate_contour():
        ...
        for mean in means:
            ...
            for sigma in sigmas:
                params = torch.cat(
                    (torch.as_tensor([mean], dtype=torch.float32), torch.as_tensor([sigma], dtype=torch.float32)),
                    0).detach()
                ...
                bound = ut.compute_policy_loss(300, config['alpha'], logps,
                                               logfactor, logq)
    ...
    for iter in range(config['num_iters']):
        ...
        if iter > 5:
            ...
            for _ in range(config['num_updates']):
                ...
                loss_policy = ut.compute_policy_loss(config['mc_samples'], config['alpha'], logps, logfactor, logq)
                ...
                for arm in range(0, 3):
                    with torch.no_grad():
                        ...
                        logps, logfactor, logq, samples = compute_log_prob(obs[arm], params, config['mc_samples'],
                                                                           ...
                                                                           config['prior_sigma2'][arm],
                                                                           config['mixture_weight_prior'][arm])
                        bound = ut.compute_policy_loss(config['mc_samples'], config['alpha'], logps,
                                                             logfactor, logq)
                        bounds.append(bound.detach().item())
